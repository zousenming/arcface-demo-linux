...
void ArcFaceDetection::initData()
{
    ...
    readSetting(
        ...
        m_rgbCameraId, m_irCameraId, m_compareModel, m_featureThreadNum
    );
	...
	while (m_isOpenCamera)
    {
		...
		if (!m_rgbFrame.empty())
		{
			...
			if (MOK == res)
			{
				...
				//添加图像质量检测
				res = m_videoEngine.PreImageQualityDetect(rgbImage, m_videoFaceInfo, m_rgbFqThreshold);
				if (MOK == res) {
					m_dataValid = true;
				} else {
					m_dataValid = false;
...
void ArcFaceDetection::frPreviewData()
{
	...
	while (m_isOpenCamera)
	{
		...
		MRESULT res = m_videoEngine.PreExtractFeature(rgbImage, singleFaceInfo, faceFeature, m_featureThreadNum);
...
void ArcFaceDetection::showRegisterThumbnail()
{
	...
	for (int i = 0; i < picPathList.size(); ++i)
	{
        ...
        // 图像质量检测，默认阈值为 0.35
		res = m_imageEngine.PreImageQualityDetect(registerImage, multiFaceInfo, m_rgbFqThreshold);
		if (MOK != res) {
			cvReleaseImage(&registerImage);
			continue;
		}
		...
		faceFeature.featureSize = FACE_FEATURE_SIZE;
		faceFeature.feature = (MByte *)malloc(faceFeature.featureSize * sizeof(MByte));
		res = m_imageEngine.PreExtractFeature(registerImage, singleFaceInfo, faceFeature, m_featureThreadNum);
...
void ArcFaceDetection::detectionRecognitionImage(IplImage* recognitionImage, ASF_SingleFaceInfo& faceInfo, QString& strInfo)
{
	...
	{
		//图像质量检测，默认阈值为0.35
		res = m_imageEngine.PreImageQualityDetect(recognitionImage, multiFaceInfo, m_rgbFqThreshold);
		if (MOK == res) 
		{
			...
			//FR
			res = m_imageEngine.PreExtractFeature(recognitionImage, faceInfo, m_recognitionImageFeature, m_featureThreadNum);
			if (MOK == res)
			{
