...
MRESULT FaceEngine::InitEngine(ASF_DetectMode detectMode, ASF_OrientPriority detectFaceOrientPriority)
{
	...
	if (ASF_DETECT_MODE_IMAGE == detectMode)
	{
		mask = ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE | 
            ASF_LIVENESS | ASF_IR_LIVENESS | ASF_IMAGEQUALITY;
	}
	else
	{
        mask = ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_LIVENESS | ASF_IR_LIVENESS | ASF_IMAGEQUALITY;
	}
...
)
{
    ...
    ASF_ImageQualityInfo imageQualityInfo = { 0 };
    MRESULT res = ASFImageQualityDetectEx(m_hEngine, &rgbOffscreen, &multiFaceInfo, &imageQualityInfo);
    if (res == MOK && imageQualityInfo.num > 0 && imageQualityInfo.faceQualityValue[0] > rgbFqThreshold)
    {
        cvReleaseImage(&cutImg);
        return MOK;
    }
...
MRESULT FaceEngine::PreExtractFeature(IplImage* image, ASF_SingleFaceInfo& faceRect, 
	ASF_FaceFeature& feature, MInt32 featureThreadNum)
{
	if (!image || image->imageData == NULL)
	{
		return -1;
